...
export class MailService {
  ...
  async sendOtp(otp: string, to: string) {
    ...
    await this.transporter.sendMail({
      from: this.configService.get<string>('SMTP_USER'),
      to: to,
      subject: 'Password Reset Token for Food Delivery Backend',
      text: `Your password reset token for Food Delivery Backend is ${token}.Please do not share this token with anyone.`,
    });
  }
}
